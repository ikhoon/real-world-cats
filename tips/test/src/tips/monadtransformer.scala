...
/**
  ...
  *
  */
object MonadTransformerSaveUs extends TestSuite {
  ...
  def toIds(ids: String): List[Long] =
    ids.split(",").map(_.toLong)(collection.breakOut)
  ...
  def findAddress(id: Long): Future[Option[Address]] =
  ...
  def findOption(itemId: Long): Future[ItemOption] =
    Future.successful(ItemOption(getId, itemId))
  def findItem(itemId: Long): Future[Item] = Future.successful(Item(itemId))
  def findOrder(userId: Long): Future[Option[Order]] =
    Future.successful(Some(Order(getId, userId, getId)))

  /** 1 : N(with Future) 외부에서 key를 가지고 여러건의 데이터를 select할 경우가 있다 */
  def findOrders(userId: Long): Future[List[Order]] =
    Future.successful(
      List(Order(getId, userId, getId), Order(getId, userId, getId)))
  def findOrderItems(orderId: Long): Future[List[OrderItem]] =
    Future.successful(
      List(OrderItem(getId, orderId), OrderItem(getId, orderId)))
  ...
  def findAllOrderItemsByUserId(userId: Long): Future[List[OrderItem]] = {
    ...
    val value2: Future[Future[List[List[OrderItem]]]] =
      value1.map(Future.sequence(_))
  ...
  def findAllOrderItemsByUserId4(
      friends: List[Long]): Future[List[OrderItem]] = {
    // 일단 우리는 먼저 `friends.map` 부터 생각한다.
    val value1: List[Future[List[OrderItem]]] =
      friends.map(findAllOrderItemsByUserId)
  ...
  def findAllOrderItemdByUserId5(
      friends: List[Long]): Future[List[OrderItem]] = {
    val value1: Future[List[List[OrderItem]]] =
      friends.traverse(findAllOrderItemsByUserId)
    val value2: Future[List[OrderItem]] = value1.map(_.flatten) // 헐 flatten이라니!
    value2
  }

  // map + sequence + flatten = flatTraverse
  def findAllOrderItemdByUserId6(
      friends: List[Long]): Future[List[OrderItem]] = {
    val value1: Future[List[OrderItem]] =
      friends.flatTraverse(findAllOrderItemsByUserId)
    value1
  }
    ...
    'monad_transformer {
      ...
      final case class MyOptionT[F[_], A](value: F[Option[A]]) {
        def flatMap[B](f: A => OptionT[F, B])(
            implicit F: Monad[F]): MyOptionT[F, B] =
          flatMapF(a => f(a).value)

        def flatMapF[B](f: A => F[Option[B]])(
            implicit F: Monad[F]): MyOptionT[F, B] = {
